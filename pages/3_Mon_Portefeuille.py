# -*- coding: utf-8 -*-
"""
Mon Portefeuille v7.2 — IA Hybride (CT + LT)
- Répartition
- Décision IA (CT) + long terme (MA120/240)
- Surbrillance proximité d'entrée
- Benchmark au choix
"""
...
st.title("💼 Mon Portefeuille — PEA & CTO (IA Hybride)")
...
# --- Tableau principal + Analyse IA (CT + LT)
...
hist_full = fetch_prices(tickers, days=360)
...
for _, r in merged.iterrows():
    ...
    if np.isfinite(px) and np.isfinite(ma120) and np.isfinite(ma240):
        if px > ma120 > ma240: lt_ind = "🌱 LT+"
        elif px < ma120 < ma240: lt_ind = "🌧 LT-"
        else: lt_ind = "⚖️ LT="
    else:
        lt_ind = "❓"
...
def highlight_near_entry(row):
    if pd.notna(row["Proximité (%)"]) and abs(row["Proximité (%)"]) <= 2:
        return ["background-color: #2b2b2b22; font-weight:600"] * len(row)  # plus lisible dark
    return [""] * len(row)
def color_decision(val):
    if pd.isna(val): return ""
    if "Acheter" in val: return "background-color: rgba(0,200,0,0.18);"
    if "Vendre" in val: return "background-color: rgba(255,0,0,0.18);"
    if "Surveiller" in val: return "background-color: rgba(0,100,255,0.18);"
    return ""
...
if hist_graph.empty or "Date" not in hist_graph.columns:
    ...
    if df:
        ...
        msg = f"✅ Votre portefeuille surperforme le {benchmark_label} de {diff:+.2f}%." if diff > 0 else f"⚠️ Sous-performance vs {benchmark_label} {abs(diff):.2f}%."
        st.markdown(f"**{msg}**")
